...
class CardholderServiceTest {
    ...
    private static class FakeCardholderDao extends CardholderDao {
        ...
        /** Lo consulta emailExistsForAnother: es la comprobación de la EDICIÓN. */
        boolean emailTakenByAnother = false;
        Cardholder inserted;
        long sequence = 77L;

        /** Del ALTA. Sin este override el falso deja pasar la llamada al DAO
            real, que abriría una conexión a Oracle: la prueba dejaría de ser
            unitaria y pasaría o fallaría según qué haya en la base. */
        @Override
        public boolean emailExists(String email) { return emailTaken; }

        /** De la EDICIÓN: el mismo correo en OTRA ficha. */
        @Override
    ...
    void registerFromFullNameSplitsAndAssigns() {
        long id = service.registerFromFullName("Diego Jarillo Estrada", "dje@empresa.com", 4L);

        assertEquals(42L, id);
        assertEquals("Diego", dao.inserted.getFirstName());
        assertEquals("Jarillo Estrada", dao.inserted.getLastName());
        assertEquals(4L, dao.inserted.getDepartmentId());
        assertEquals("DJE0077", dao.inserted.getEmployeeCode());
    }
    ...
    void registerFromFullNameRejectsASingleWord() {
        ValidationException e = assertThrows(ValidationException.class,
                () -> service.registerFromFullName("Cher", "cher@empresa.com", 4L));
    ...
    void updateRejectsAOneWordName() {
        ValidationException e = assertThrows(ValidationException.class,
                () -> service.update(1L, "Ana", "ana@empresa.com", 4L, null));
        assertTrue(e.getErrors().get(0).contains("apellido"));
    }
    ...
    void updateRejectsAnEmailThatBelongsToSomeoneElse() {
        ...
        assertThrows(ValidationException.class,
                () -> service.update(1L, "Ana López", "otro@empresa.com", 4L, null));
    }
